...
function idCheck(form) {
    ...
    if (_id == '') {
        ...
        $('#verify_id').prop("type", "hidden");
		$('#reject_id').prop("type", "hidden");
		$('#check_id').prop("type", "button");
        return;
    }
    ...
    if (!idReg.test(input_id.value)) {
        ...
        form.input_id.focus();
        $('#verify_id').prop("type", "hidden");
		$('#reject_id').prop("type", "button");
		$('#check_id').prop("type", "hidden");
        return;
    }
    ...
    $.ajax({
        ...
        success : function(data, textStatus) {
            if (data == 'false') {
                ...
                $('#verify_id').prop("type", "button");
				$('#reject_id').prop("type", "hidden");
				$('#check_id').prop("type", "hidden");
				$('#verify_id').prop("disabled", "disabled");
            } else {
...
function emailCheck() {
    ...
    var ori_email = $("#input_ori_email").val()
	var url = contextPath+"/member/checkEmail.do";
	var form = document.frmJoin;
	
    if (_email == '') {
        popAlert("이메일 입력", "이메일을 입력해 주세요.", "warning", "확인");
        $('#verify_email').prop("type", "hidden");
		$('#reject_email').prop("type", "hidden");
		$('#check_email').prop("type", "button");
        return;
    }
    ...
    if (!emailReg.test(input_email.value)) {
        ...
        $('#verify_email').prop("type", "hidden");
		$('#reject_email').prop("type", "button");
		$('#check_email').prop("type", "hidden");
        return;
    }
    ...
    $.ajax({
        ...
        success : function(data, textStatus) {
            ...
            if (data == 'false') {
				...
				$('#verify_email').prop("type", "button");
				$('#reject_email').prop("type", "hidden");
				$('#check_email').prop("type", "hidden");
				$('#verify_email').prop("disabled", "disabled");
            }
